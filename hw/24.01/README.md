code: https://github.com/maksim0840/kpo-2025/tree/sem2-sem3_hw/practice/practise-2
...
- SOLID :: Single-responsibility principle (Принцип единственной ответственности) - класс должен иметь только одну причину для изменения; класс должен отвечать только за что-то одно
```
...
- SOLID :: Open–closed principle (Принцип открытости-закрытости) - объекты должны быть открыты для расширения, но закрыты для модификации.
```
...
- SOLID :: Liskov substitution principle (Принцип подстановки Лисков) - классы наследники должны полностью уметь заменять своих родителей.
```
...
- SOLID :: Interface segregation principle (Принцип разделения интерфейсов) - разделять интерфейсы, не заставлять классы реализовывать методы, которые они не используют 
```
...
- SOLID :: Dependency inversion principle (Принцип инверсии зависимостей) -  зависимости должны строиться на основе абстракций, а не конкретных классов
```
